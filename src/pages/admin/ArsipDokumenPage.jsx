...
import { UploadCloud, Filter, Trash2, Briefcase, Download, Eye, FileText } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { id } from 'date-fns/locale';
import { formatFileSize } from '../../utils/formatter'; // Pastikan path ini benar
...
const ArsipDokumenPage = () => {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            title: 'Anda yakin?', text: "Dokumen yang dihapus tidak dapat dikembalikan!", icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
            confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal'
        }).then(async (result) => {
            if (result.isConfirmed) {
                try {
                    await apiClient.delete(`/arsip/${id}`);
