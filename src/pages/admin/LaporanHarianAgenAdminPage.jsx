// src/pages/admin/LaporanHarianAgenAdminPage.jsx
...
// Helper dan state awal bisa dicopy dari halaman agen
...
// Komponen Form Fields
const FormFields = ({ formData, handleChange }) => (
    <div className="space-y-4">
        {/* Konten form sama persis dengan yang ada di halaman agen */}
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-4 bg-gray-50 rounded-lg space-y-2"><h4 className="font-medium text-center">Gadai</h4><input type="number" name="gadaiPot" value={formData.gadaiPot || ''} onChange={handleChange} placeholder="POT" className="w-full p-2 border rounded-md" /><input type="number" name="gadaiOsl" value={formData.gadaiOsl || ''} onChange={handleChange} placeholder="OSL (Rp)" className="w-full p-2 border rounded-md" /></div>
                <div className="p-4 bg-gray-50 rounded-lg space-y-2"><h4 className="font-medium text-center">Mulia</h4><input type="number" name="muliaPot" value={formData.muliaPot || ''} onChange={handleChange} placeholder="POT" className="w-full p-2 border rounded-md" /><input type="number" name="muliaOsl" value={formData.muliaOsl || ''} onChange={handleChange} placeholder="OSL (Rp)" className="w-full p-2 border rounded-md" /></div>
                <div className="p-4 bg-gray-50 rounded-lg space-y-2"><h4 className="font-medium text-center">Mikro</h4><input type="number" name="mikroPot" value={formData.mikroPot || ''} onChange={handleChange} placeholder="POT" className="w-full p-2 border rounded-md" /><input type="number" name="mikroOsl" value={formData.mikroOsl || ''} onChange={handleChange} placeholder="OSL (Rp)" className="w-full p-2 border rounded-md" /></div>
                <div className="p-4 bg-gray-50 rounded-lg space-y-2"><h4 className="font-medium text-center">Lainnya</h4><input type="text" name="lainnyaNamaProduk" value={formData.lainnyaNamaProduk || ''} onChange={handleChange} placeholder="Nama Produk" className="w-full p-2 border rounded-md mb-2" /><input type="number" name="lainnyaPot" value={formData.lainnyaPot || ''} onChange={handleChange} placeholder="POT" className="w-full p-2 border rounded-md" /><input type="number" name="lainnyaOsl" value={formData.lainnyaOsl || ''} onChange={handleChange} placeholder="OSL (Rp)" className="w-full p-2 border rounded-md" /></div>
            </div>
        </div>
    </div>
);
...
const LaporanHarianAgenAdminPage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        // Admin hanya bisa mengedit, tidak membuat laporan atas nama agen
        if (!currentLaporan) {
            Swal.fire('Info', 'Admin hanya dapat mengedit laporan yang sudah ada.', 'info');
            return;
        }
        try {
            const response = await apiClient.put(`/laporan-harian-agen/${currentLaporan.id}`, formData);
    ...
    const handleExport = async () => {
        try {
            ...
            link.href = url;
            link.setAttribute('download', `Laporan_Harian_Semua_Agen_${new Date().toISOString().slice(0,10)}.xlsx`);
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-md">
                 <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-2xl font-bold text-gray-800">Kelola Laporan Harian Agen</h1>
                    </div>
                    <button onClick={handleExport} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">
                        <Download size={18}/> Export Semua Laporan
                    </button>
                </div>
            </div>

            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Edit Laporan Harian Agen">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                <h2 className="text-xl font-bold mb-4">Data Laporan Masuk</h2>
